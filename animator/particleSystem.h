...
class Particle
{
	...
	Particle(){
		...
		pos = Vec3f(0.5, 0.6+2*h1, 0.6);
		prePos = Vec3f(0.5, 0.6+2*h1, 0.6);
		speed = Vec3f(0.0, 1.0, 0.0);
		preSpeed = Vec3f(0.0, 1.0, 0.0);
		//std::cout<<"T"<<std::endl;
		life = 0.0;
	}
};
...
protected:
	...
	std::vector<Particle> ptcs;//, ptcsG;
	Vec3f windForce = Vec3f(1, 1, 1)*30;
	bool firstTime = true;
